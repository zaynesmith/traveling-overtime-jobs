...
export default function CandidateCard({
  ...
  const resolvedProfileId = profileId || jobseekerId || id || null;
  const profileHref = resolvedProfileId ? `/dashboard/employer/jobseeker/${resolvedProfileId}` : null;
    ...
    <li className="rounded-2xl bg-white p-5 shadow-lg">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          {profileHref ? (
            <Link href={profileHref} className="inline-flex items-center gap-2 text-sm font-semibold text-sky-600">
              See Profile
              <svg
                aria-hidden="true"
                className="h-4 w-4"
